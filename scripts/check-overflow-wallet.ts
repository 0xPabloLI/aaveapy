...
async function main() {
  ...
  const info = await page.evaluate(() => {
    ...
    const results: string[] = [
      `Panel: x=${pr.x.toFixed(1)} w=${pr.width.toFixed(1)} right=${pr.right.toFixed(1)} class=${(panel.className || '').slice(0, 100)}`,
    ];
    ...
    for (const el of allInteractive) {
      ...
      const tag =
        el.tagName +
        (el.getAttribute('data-testid') || el.getAttribute('aria-label') || el.textContent?.slice(0, 20) || '');
      results.push(
        `  ${tag}: x=${r.x.toFixed(1)} w=${r.width.toFixed(1)} right=${r.right.toFixed(1)} h=${r.height.toFixed(1)} overflows=${overflowsPanel}`,
      );
    }
    return results;
  });
