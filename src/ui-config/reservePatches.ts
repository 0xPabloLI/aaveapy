...
if (Array.isArray(tokenListTokens)) {
  for (const token of tokenListTokens) {
    if (token.logoURI) {
      tokenLogoByAddress.set(token.address.toLowerCase(), token.logoURI);
    }
  }
}
...
export function fetchIconSymbolAndName({ underlyingAsset, symbol, name }: IconSymbolInterface) {
  ...
  if (lowerUnderlyingAsset && Object.prototype.hasOwnProperty.call(underlyingAssetMap, lowerUnderlyingAsset)) {
    return {
      symbol,
