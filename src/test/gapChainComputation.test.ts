...
describe('computeGapChainIds', () => {
  it('returns empty gap when reserves only cover SDK-registered chains', () => {
    const reserves = [...AAVE_V3_CHAIN_IDS.map(v3Entry), ...AAVE_V4_CHAIN_IDS.map(v4Entry)];
  ...
  it('deduplicates gap chains by chainId', () => {
    const gapChain = 4326;
    const result = computeGapChainIds([v3Entry(gapChain), v3Entry(gapChain)], {
      v3SdkChainIds: [],
      v4SdkChainIds: [9999],
    });
    const total =
      result.v3Gap.filter((id) => id === gapChain).length + result.v4Gap.filter((id) => id === gapChain).length;
    expect(total).toBe(1);
  });

  it('classifies gap chain as V3 via marketName prefix', () => {
    const result = computeGapChainIds([v3Entry(99999)], { v3SdkChainIds: [1], v4SdkChainIds: [] });
    expect(result.v3Gap).toContain(99999);
    expect(result.v4Gap).toEqual([]);
  });

  it('classifies gap chain as V4 via marketName prefix', () => {
    const result = computeGapChainIds([v4Entry(99999)], { v3SdkChainIds: [], v4SdkChainIds: [42161] });
