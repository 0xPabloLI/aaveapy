...
describe('getReserveKey', () => {
  it('returns V3 reserveId as-is', () => {
    expect(getReserveKey({ reserveId: '1:0x8787:0xA0b8' })).toBe('1:0x8787:0xA0b8');
  });

  it('returns V4 reserveId with hubName suffix as-is', () => {
    expect(getReserveKey({ reserveId: '1:0x8787:0xA0b8:Core' })).toBe('1:0x8787:0xA0b8:Core');
  });
...
describe('buildReserveLookupByChainAndToken', () => {
  ...
  it('skips reserves without chainId or tokenAddress', () => {
    const incomplete = [{ reserveId: 'no-chain' }, { chainId: 1, reserveId: 'no-token' }] as ReserveWithSpread[];
...
describe('buildReserveMap', () => {
  ...
  it('trims whitespace on reserveId keys', () => {
    const spaced = [{ reserveId: '  1:0xp:0xaaa  ' }] as ReserveWithSpread[];
...
describe('composeReserveId', () => {
  ...
  it('composes V4 reserveId with hubAddress: chainId:poolAddress:tokenAddress:hubAddress', () => {
    expect(composeReserveId(1, '0x8787...', '0xA0b8...', '0xCca8A2316a28C583e12c8844d2D4E1f4D8F8D26c9')).toBe(
      '1:0x8787...:0xa0b8...:0xcca8a2316a28c583e12c8844d2d4e1f4d8f8d26c9',
    );
  });
