...
export const parseCampaignBoundaryMs = (value: string | undefined, boundary: 'start' | 'end'): number | null => {
...
>(
  groups?: TGroup[],
): Array<FlattenedCampaignBreakdown<TGroup, TBreakdown>> => {
  if (!groups?.length) return [];
  return groups.flatMap((group) => (group.breakdowns ?? []).map((breakdown) => ({ group, breakdown })));
};
...
export const sumActiveCampaignBreakdownValues = <TGroup, TBreakdown>(
...
export const applyStableCampaignLabels = <TRow extends { label: string }>(rows: TRow[]): TRow[] => {
