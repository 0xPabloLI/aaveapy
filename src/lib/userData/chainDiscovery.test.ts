...
describe('discoverUnregisteredChains (bulk API)', () => {
  it('skips registered chains and discovers unregistered ones from bulk endpoint', async () => {
    ...
    global.fetch = vi
      ...
      .mockResolvedValueOnce(
        mockBulkChainIdNetwork([{ chainId: 4326, name: 'MegaETH', rpc: ['https://mainnet.megaeth.com/rpc'] }]),
      )
      .mockResolvedValueOnce(
        mockBulkChainlistOrg([{ chainId: 4326, name: 'MegaETH', rpc: [{ url: 'https://megaeth.drpc.org' }] }]),
      );
        ...
        mockBulkChainIdNetwork([
          {
            chainId: 4326,
            name: 'MegaETH',
            rpc: ['https://mainnet.megaeth.com/rpc', 'https://megaeth.drpc.org', 'https://static-rpc.megaeth.com'],
          },
        ]),
      )
      .mockResolvedValueOnce(
        mockBulkChainlistOrg([
          {
            chainId: 4326,
            name: 'MegaETH',
            rpc: [{ url: 'https://megaeth.drpc.org' }, { url: 'https://mainnet.megaeth.com/rpc' }],
          },
        ]),
      );
  ...
  it('handles chain not found in bulk data', async () => {
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce(mockBulkChainIdNetwork([]))
      .mockResolvedValueOnce(mockBulkChainlistOrg([]));
  ...
  it('merges wagmi/chains RPC URLs with bulk data (static > wagmi > bulk)', async () => {
    ...
    global.fetch = vi
      .fn()
      .mockResolvedValueOnce(
        mockBulkChainIdNetwork([{ chainId: 4326, name: 'MegaETH', rpc: ['https://megaeth.drpc.org'] }]),
      )
      .mockResolvedValueOnce(mockBulkChainlistOrg([]));
