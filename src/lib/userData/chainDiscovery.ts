...
function findInBulkCache(
  chainId: number,
): { rpcUrls: string[]; name: string; nativeCurrency?: { name: string; symbol: string; decimals: number } } | null {
  let rpcUrls: string[] = [];
...
export async function discoverUnregisteredChains(chainIds: number[]): Promise<void> {
  const unregisteredChainIds = chainIds.filter((chainId) => !isRegisteredChain(chainId));
  ...
  for (const chainId of unregisteredChainIds) {
    ...
    const rpcUrls = mergeRpcUrls(staticRpcUrls, wagmiRpcUrls, bulkResult ? bulkResult.rpcUrls : []);
