...
const makeReserve = (
  overrides: Partial<ReserveWithSpread> & { reserveId: string; chainId: number },
): ReserveWithSpread => ({
...
const makeWalletPos = (
  overrides: Partial<WalletPosition> & { reserveId: string; chainId: number; side: 'supply' | 'borrow' },
): WalletPosition => ({
...
const reserves: ReserveWithSpread[] = [
  ...
  makeReserve({
    reserveId: 'eth-usdc-v4',
    chainId: 1,
    marketName: 'AaveV4Ethereum',
    chainName: 'Ethereum',
    tokenSymbol: 'USDC',
    spokeAddress: '0xV4Spoke',
    hubName: 'Core',
    hubId: 'hub-core',
  }),
];
