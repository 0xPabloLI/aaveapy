...
describe('formatPortfolioAmount', () => {
  ...
  it('clips every noisy float to ≤ MAX_PORTFOLIO_AMOUNT_SIG_DIGITS sig digits', () => {
    for (const v of NOISY) {
      ...
      expect(countSignificantDigits(out), `value=${v} → "${out}"`).toBeLessThanOrEqual(MAX_PORTFOLIO_AMOUNT_SIG_DIGITS);
    }
  });
...
const wallet: WalletPosition[] = [
  {
    ...
    side: 'supply',
    tokenSymbol: 'WETH',
    amountUsd: Number('1737.4839284729384'),
    isOrphan: false,
    source: 'sdk',
  } as unknown as WalletPosition,
  {
    ...
    side: 'borrow',
    tokenSymbol: 'GHO',
    amountUsd: Number('412.91827361827361'),
    isOrphan: false,
    source: 'sdk',
  } as unknown as WalletPosition,
];
...
describe('Portfolio amount entry paths use formatPortfolioAmount', () => {
  it('wallet → portfolio conversion clips amounts', () => {
    ...
    expectAllClipped(
      out.flatMap((e) => [e.supply.amount, e.borrow.amount].filter((a) => a !== '')),
      'walletPositionToEntries',
    );
  });
});
