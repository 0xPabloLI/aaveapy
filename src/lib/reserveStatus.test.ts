...
describe('hasProtocolRestriction (internal via isSupplyDisabled)', () => {
  ...
  it('V3 frozen reserve (no isActive field) → not treated as inactive', () => {
    const r = { ...BASE_RESERVE, isFrozen: true, isPaused: false };
    // isActive undefined → not counted as inactive
    expect(isSupplyDisabled(r)).toBe(true);
  });
});
