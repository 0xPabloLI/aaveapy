  ...
  const connector = createConnector((config) => {
    ...
    return {
      ...
      async connect({ chainId }: { chainId?: number } = {}) {
        const address = watchAddress ?? (localStorage.getItem(STORAGE_KEY) as `0x${string}` | null) ?? undefined;
      ...
      async isAuthorized() {
        const address = watchAddress ?? (localStorage.getItem(STORAGE_KEY) as `0x${string}` | null);
        return !!address;
      },
