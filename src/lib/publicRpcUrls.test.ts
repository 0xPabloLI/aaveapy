...
import { getPublicRpcUrls, PUBLIC_RPC_URLS, AAVE_CHAIN_IDS } from './publicRpcUrls';

describe('publicRpcUrls', () => {
  describe('PUBLIC_RPC_URLS', () => {
    it('covers all Aave mainnet chain IDs', () => {
      const expectedChainIds = [
        1, 10, 56, 100, 137, 146, 196, 324, 1868, 42220, 5000, 8453, 1088, 59144, 42161, 43114, 534352,
      ];
      for (const chainId of expectedChainIds) {
        expect(PUBLIC_RPC_URLS[chainId], `missing public RPC for chain ${chainId}`).toBeDefined();
    ...
    it('every URL is a valid public HTTP(S) endpoint (no API keys)', () => {
      for (const [chainId, urls] of Object.entries(PUBLIC_RPC_URLS)) {
        for (const url of urls) {
          ...
          expect(url, `chain ${chainId}: URL contains API key placeholder "${url}"`).not.toContain('${');
        }
      }
    });
  });
  ...
  describe('AAVE_CHAIN_IDS', () => {
    it('contains all V3 mainnet chain IDs', () => {
      const v3ChainIds = [
        1, 42161, 43114, 56, 8453, 42220, 1, 100, 59144, 5000, 1088, 10, 137, 534352, 1868, 146, 196, 324,
      ];
