...
import { computeDeficitDisplay, type DeficitDisplay } from '@/lib/deficit';
...
const marketCellClassNames = {
  ...
  marketButton:
    'inline-flex items-center justify-center gap-[var(--ds-space-1-5)] px-[var(--ds-space-2)] py-[var(--ds-space-1)] transition-colors duration-150 hover:text-foreground hover:bg-card/80 active:scale-[0.98]',
  ...
  externalLink:
    'pointer-events-none absolute right-0 top-1/2 inline-flex -translate-y-1/2 items-center justify-center opacity-0 transition-opacity duration-100',
};
...
function SortArrowButton({
  ...
  ariaLabel,
  className,
}: SortArrowButtonProps & { className?: string }) {
  return (
    <button
      type="button"
      onClick={(e) => {
...
const DesktopReserveRow = memo(
  ({
    ...
    const deficitDisplay: DeficitDisplay = computeDeficitDisplay(
      reserve,
      displayTokenPrice,
      displayReserveSizeUsd,
      inputMode,
    );
    const deficitUsdLabel =
      deficitDisplay.deficitUsd != null ? formatUsd(deficitDisplay.deficitUsd) : '— (token price unavailable)';
    ...
    const supplySizeSortArrow = (
      <SortArrowButton
        onClick={sortActions.supply.onSort}
        isActive={sortActions.supply.isActive}
        sortOrder={sortActions.supply.sortOrder}
    ...
    const borrowSizeSortArrow = (
      <SortArrowButton
        onClick={sortActions.borrow.onSort}
        isActive={sortActions.borrow.isActive}
        sortOrder={sortActions.borrow.sortOrder}
        ...
        >
          ...
          <TableCell className="ds-reserves-cell-td-edge-l ds-row-pad text-left overflow-hidden">
            <div className="flex w-full min-w-0 items-center justify-start gap-[var(--ds-space-2)]">
                  ...
                  const btn = (
                    <button
                      ...
                      aria-label={
                        isInPortfolio
                          ? isHidden
                            ? `Restore ${reserve.tokenSymbol} to portfolio`
                            : `Remove ${reserve.tokenSymbol} from portfolio`
                          : `Add ${reserve.tokenSymbol} to portfolio`
                      }
                    >
                      {isInPortfolio ? (
                        isHidden ? (
              ...
              <div
                className={`group/token flex min-w-0 max-w-full justify-start gap-[var(--ds-space-1-5)] ${isTokenWrapped ? 'items-start' : 'items-center'}`}
              >
                <TokenIcon
                  symbol={iconSymbol}
                  size={28}
                  loading="eager"
                  logoURI={logoURI}
                  className={`shrink-0 ${isTokenWrapped ? 'mt-0.5' : ''}`}
                />
                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-[var(--ds-space-1-5)] gap-y-0">
                  <FrozenStatusBadge reserve={reserve} />
                  <span
                    ref={tokenTextRef}
                    className="font-semibold text-foreground ds-text-13 break-words min-w-0 [max-width:max-content]"
                  >
                    {reserve.tokenSymbol}
                  </span>
                    ...
                    {aaveV4HubUrl && (
                      <a
                        ...
                        className={cn(
                          ...
                          'group-hover/hub-link:pointer-events-auto group-hover/hub-link:opacity-100',
                        )}
                        aria-label={`View ${reserve.hubName} hub on Aave V4`}
                        title={`Open hub ${reserve.hubName} on Aave V4`}
                      >
                        <ExternalLink
                          className={cn('w-2.5 h-2.5', isV4Market ? 'text-foreground' : 'text-muted-foreground')}
                        />
                      </a>
                    )}
                  </div>
                      ...
                      >
                        <div className="flex flex-col">
                          ...
                          >
                            ...
                            <img
                              src={isDark ? '/icons/partners/tydro-white.svg' : '/icons/partners/tydro-black.svg'}
                              alt="Tydro"
            ...
            <div className="flex flex-col items-end justify-center gap-[var(--ds-space-0-5)]">
              ...
              {hasSupplyCap ? (
                ...
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div
                      className={`inline-flex items-center gap-[var(--ds-space-1-5)] rounded-md py-0.5 pl-1 pr-0.5 -my-0.5 cursor-pointer ${supplyBlocked ? 'text-emerald-500/50' : 'ds-text-emerald-500'}`}
                    >
              ...
              {hasBorrowCap ? (
                ...
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div
                      className={`inline-flex items-center gap-[var(--ds-space-1-5)] rounded-md py-0.5 pl-1 pr-0.5 -my-0.5 cursor-pointer ${borrowBlocked ? 'text-cyan-500/50' : 'ds-text-brand-cyan'}`}
                    >
                ...
                (deficitDisplay.deficitUsd != null ? (
                  <DeficitLiquidityRing
                    ...
                    label={
                      <span
                        className={cn(
                          'inline-flex items-center gap-1 ds-text-11 tabular-nums',
                          deficitDisplay.deficitTextClass,
                        )}
                      >
                        <DeficitShieldIcon
                          ratio={deficitDisplay.deficitShareRatio}
                          className={cn(deficitDisplay.isNeutralDeficit && 'opacity-70')}
                        />
                ...
                <TooltipTrigger asChild>
                  <div className="inline-flex items-center justify-end gap-[var(--ds-space-1-5)] cursor-default">
                    <div className="flex flex-col items-end gap-[var(--ds-space-0-5)]">
                      <span
                        className={`ds-text-13 font-bold tabular-nums ${
                          availableLiquidityUsd != null && availableLiquidityUsd < LOW_LIQUIDITY_THRESHOLD_USD
                            ? 'text-amber-600'
                            : 'ds-text-purple-600'
                        }`}
                      >
                        {formatScenarioSize(availableLiquidityUsd, {
                          inputMode,
                          tokenPrice: displayTokenPrice,
                          tokenSymbol: reserve.tokenSymbol,
                        })}
                    ...
                    <UtilizationIndicator current={displayUtilization} optimal={optimalPct} />
                  </div>
                </TooltipTrigger>
                <TooltipContent
                  side="bottom"
                  className="w-[min(var(--ds-ring-tooltip-max-w),calc(100vw-2rem))] max-w-[var(--ds-ring-tooltip-max-w)] overflow-hidden p-3"
                >
                  <TooltipCalloutArrow />
                  <UtilizationContent
            ...
            <div className="flex flex-col items-end justify-center gap-[var(--ds-space-0-5)] min-h-[2.75rem]">
              ...
              {displaySupplyIncentive !== null ? (
                <div className="flex items-center gap-[var(--ds-space-0-5)] ds-text-11 justify-end min-h-[1.25rem]">
                  <span
                    className={`tabular-nums font-medium ${supplyBlocked ? 'text-emerald-500/40' : 'ds-text-emerald-500-70'}`}
                  >
              ...
              {displayBorrowIncentive !== null ? (
                <div className="flex items-center gap-[var(--ds-space-0-5)] ds-text-11 justify-end min-h-[1.25rem]">
                  {displayBorrowNative !== null && (
                    <>
                      <span
                        className={`tabular-nums font-medium ${borrowBlocked ? 'text-cyan-500/40' : 'ds-text-brand-cyan-70'}`}
                      >
            ...
            >
              <div className="overflow-hidden">
                {hasSimulationMounted && simulation && (
                  <div data-reserves-simulation-scrollport className="px-[var(--ds-space-3)] py-[var(--ds-space-3)]">
                    <SimulationSubRow
                      reserve={reserve}
