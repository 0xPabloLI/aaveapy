...
export function DeficitProgressContent({
  ...
  const deficitAmountArrow = onSortDeficitAmount ? (
    <SortArrowButton
      onClick={onSortDeficitAmount}
      isActive={!!isSortDeficitAmountActive}
      sortOrder={deficitAmountSortOrder}
      ariaLabel="Sort by deficit amount"
      className={getProgressColorClass()}
    />
  ) : null;
  ...
  const deficitDisplayValue =
    displayMode === 'token' ? (tokenDeficitLabel ?? '—') : formatScenarioSize(deficitUsd, { inputMode: 'usd' });
  const totalSuppliedDisplayValue =
