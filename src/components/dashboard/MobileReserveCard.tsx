...
function MobileCapSheet({
  ...
  const CAP_SHEET_CONTENT: Record<string, React.ReactNode> = {
    ...
    utilization:
      optimalPct != null && displayUtilization != null ? (
        <UtilizationContent
          current={displayUtilization}
          optimal={optimalPct}
          formulaVariant="inline"
          formulaLabel="Utilization"
        />
      ) : null,
    deficit: hasDeficit ? (
    ...
    frozen: <FrozenStatusContent reserve={reserve} />,
  };
...
function MobileReserveAmountRow({
  ...
  const priceEl =
    tp != null && Number.isFinite(tp) ? (
      <span className="ds-text-11 text-muted-foreground/60 tabular-nums shrink-0 leading-none">{formatUsd(tp)}</span>
    ) : null;
  ...
  return (
    <div className="flex w-full min-w-0 flex-nowrap items-center gap-1.5 px-4">
      <span
        className={`ds-text-11 tabular-nums shrink-0 leading-none ${spread !== null ? 'ds-text-purple-600' : 'text-muted-foreground/60'}`}
      >
        {formatSpread(spread)}
      </span>
      <div className="ml-auto flex min-w-0 items-center justify-end gap-1">
...
function MobileReserveHeroApy({
  ...
  const noIncentivePlaceholder = (
    <span className="ds-text-11 font-medium leading-none text-muted-foreground/55">{`Base ${rateUnitLabel} only`}</span>
  );
    ...
    return (
      <div className="flex flex-col items-center gap-0.5">
        {isDisabled ? (
          ...
          <p className={`ds-text-22 font-bold tabular-nums ${heroColorClass}`}>{formatPercent(heroValue)}</p>
        )}
        <div className="flex min-h-[1rem] items-center justify-center">
          {visibleSupplyIncentive !== null ? (
            ...
            <span className="ds-text-10 font-medium leading-none text-muted-foreground/55">Supply unavailable</span>
          ) : (
  ...
  return (
    <div className="flex flex-col items-center gap-0.5">
      {isDisabled ? (
        <p className={`ds-text-22 font-bold tabular-nums ${heroColorClass} cursor-auto`}>{formatPercent(heroValue)}</p>
      ) : (
        <p className={`ds-text-22 font-bold tabular-nums ${heroColorClass}`}>{formatPercent(heroValue)}</p>
      )}
      <div className="flex min-h-[1rem] items-center justify-center">
        {visibleBorrowIncentive !== null ? (
          ...
          <span className="ds-text-10 font-medium leading-none text-muted-foreground/55">Borrow unavailable</span>
        ) : (
          noIncentivePlaceholder
        )}
      </div>
...
const MobileReserveCard = memo(
  ({
    ...
    const displayUtilization = useSpreadAfter ? (simulation.utilization.after ?? baseUtilization) : baseUtilization;

    return (
      <div
        data-reserve-id={reserveId}
        className={isSimulationExpanded && !showUpperOnly ? 'shadow-sm rounded-xl border border-border/60 bg-card' : ''}
      >
        {/* Card upper part */}
        <div
          ...
          <div className="flex items-start gap-[var(--ds-space-2)] mb-1.5 min-h-[36px] px-3">
            <div className="flex items-start gap-1 min-w-0 flex-1">
              {reserve.isFrozen || reserve.isPaused || reserve.isActive === false ? (
                ...
                <TokenIcon symbol={iconSymbol} size={28} loading="eager" className="shrink-0" logoURI={logoURI} />
              )}
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-1.5">
                  ...
                  {displayUtilization != null && optimalPct != null && (
                    ...
                    >
                      <span
                        className={`ds-text-11 font-medium tabular-nums leading-none ${
                          optimalPct != null && displayUtilization > optimalPct ? 'text-amber-600' : 'text-foreground'
                        }`}
                      >
                        {formatPercent(displayUtilization)}
                      </span>
                      <UtilizationIndicator current={displayUtilization} optimal={optimalPct} width={8} height={16} />
                    </button>
                  )}
                </div>
                    ...
                    <DeficitLiquidityRing
                      ...
                      label={
                        <DeficitShieldIcon
                          ...
                          className={cn('h-2.5 w-2.5', deficitDisplay.isNeutralDeficit && 'opacity-70')}
                        />
                      }
                      disableTooltip
                    />
            ...
            <div className="mt-1.5 px-3">
              ...
              >
                ...
                <span className="flex items-center gap-1">
                  ...
                  <span
                    className={`ds-text-10 font-medium tabular-nums ${availableLiquidityUsd !== null && availableLiquidityUsd !== undefined ? (availableLiquidityUsd < LOW_LIQUIDITY_THRESHOLD_USD ? 'text-amber-600' : 'text-purple-500') : 'text-muted-foreground/70'}`}
                  >
                    {formatScenarioSize(availableLiquidityUsd ?? null, {
                      inputMode,
                      tokenPrice: displayTokenPrice,
                      tokenSymbol: reserve.tokenSymbol,
                    })}
                ...
                <ListCollapse
                  className={`h-3.5 w-3.5 shrink-0 transition-transform duration-300 ease-in-out ${isSimulationExpanded ? 'rotate-180' : ''}`}
                />
              </button>
            </div>
          </div>
