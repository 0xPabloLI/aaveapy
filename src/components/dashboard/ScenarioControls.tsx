...
function ScenarioInputField({
  ...
  const placeholder =
    side === 'supply' ? (inputMode === 'usd' ? '100,000' : '50') : inputMode === 'usd' ? '20,000' : '10';
    ...
    <div className={`flex items-center ${wrapperGap} ${wrapperExtras}`}>
      ...
      <div className="relative flex-1 min-w-0">
        <input
          ...
          className={cn(inputBase, cnDsInputSurface(hasValue, side), 'min-w-0 w-full', hasValue ? clearPr : '')}
...
const ScenarioControls = memo(
  forwardRef<ScenarioControlsHandle, ScenarioControlsProps>(
    (
      {
        onDebouncedChange,
      ...
      const batchCommit = useCallback(
        (side: 'supply' | 'borrow', formattedValue: string) => {
          if (side === 'supply') {
              ...
              <div className="absolute left-0 right-0 top-full z-10 mt-1 rounded-xl border border-border/60 bg-card/95 px-1.5 py-1 backdrop-blur-sm">
                ...
                <div className="mt-1 border-t border-border/20 pt-1">
                  <div className="space-y-0.5 px-0.5 text-left text-muted-foreground/60 ds-text-10 leading-snug">
                    <p>
                      <span className="text-muted-foreground/75">Net on:</span> overlapping supply and borrow offset
                      each other first.
                    </p>
                    <p>
                      <span className="text-muted-foreground/75">Net off:</span> both sides are counted in full, which
                      may overestimate incentives.
                    </p>
