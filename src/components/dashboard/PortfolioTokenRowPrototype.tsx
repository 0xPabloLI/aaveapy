...
import { Eraser, Minus, EyeOff, Snowflake, PauseCircle, Ban, AlertTriangle, Lock } from 'lucide-react';
...
const cnModeBadgeBase = 'shrink-0 rounded border border-border/40 bg-muted/60 px-1.5 py-0.5 ds-text-10 font-semibold text-muted-foreground';
const cnDisabledInput = 'h-[var(--ds-chip-h)] w-full min-w-[4rem] rounded-md ds-text-12 tabular-nums placeholder:italic cursor-not-allowed border border-border/30 bg-muted/30 text-muted-foreground';

interface SideInputPrototypeProps {
...
function useSideInputLogic({
  sideData, side, sideLabel, tokenSymbol, tokenPriceInUsd, isMobile,
  reserveId, actions, disabled, disabledNotice, capLimitUsd,
}: Omit<SideInputPrototypeProps, 'variant'>) {
  ...
  const effectiveDisplay = sideData.inputMode === 'usd'
    ...
    : sideData.amount;
  const deltaUsdForSign = effectiveUsdForSign - (sideData.walletValue ?? 0);
  const walletDisplay = hasWallet ? formatNumberInput(formatConvertedAmount(sideData.walletValue!)) : '';
  ...
  return {
    isBorrow, labelColor, inputVariant, hasWallet, hasValue, isPositiveDelta,
    numberInput, handleDeltaCommit, toggleDeltaSign, handleToggleInputMode,
    effectiveDisplay, walletDisplay, isModified, priceUnavailable,
    sideData, side, sideLabel, tokenSymbol, tokenPriceInUsd, isMobile,
    reserveId, actions, disabled, disabledNotice, capLimitUsd,
  };
}

function SideInputA(props: SideInputPrototypeProps) {
  const logic = useSideInputLogic(props);
  const { hasWallet, hasValue, isPositiveDelta, numberInput, handleDeltaCommit,
    toggleDeltaSign, handleToggleInputMode, effectiveDisplay, walletDisplay,
    isModified, priceUnavailable, isBorrow, labelColor, inputVariant,
    sideData, sideLabel, tokenSymbol, tokenPriceInUsd, disabled, disabledNotice } = logic;

  if (disabled) {
    return (
      <div className="flex min-w-0 flex-1 items-center gap-1.5 opacity-40 cursor-not-allowed">
        <span className={cn('shrink-0 ds-text-12 font-semibold', logic.isMobile ? 'w-10' : 'w-11', labelColor)}>{sideLabel}</span>
        <span className={cnModeBadgeBase}>
          {sideData.inputMode === 'usd' ? '$' : 'T'}
        </span>
        <input value={sideData.amount} readOnly placeholder="—" className={cnDisabledInput} aria-label={`${sideLabel} (disabled) for ${tokenSymbol}`} />
      </div>
    );
  }
    ...
    <div className="flex min-w-0 flex-1 items-center gap-1.5">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button type="button" disabled={tokenPriceInUsd === undefined} onClick={handleToggleInputMode}
            className={cn('shrink-0 rounded border border-border/40 bg-muted/60 px-1.5 py-0.5 ds-text-10 font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground', tokenPriceInUsd === undefined && 'opacity-40 cursor-not-allowed hover:bg-muted/60 hover:text-muted-foreground')}
            aria-label={`Switch to ${sideData.inputMode === 'usd' ? 'token' : 'USD'} input`}>
            {sideData.inputMode === 'usd' ? '$' : 'T'}
          </button>
        </TooltipTrigger>
        {tokenPriceInUsd === undefined && <TooltipContent side="top" className="ds-text-11">Price unavailable</TooltipContent>}
      </Tooltip>
      {hasWallet && (
        <Tooltip>
          <TooltipTrigger asChild>
            <span className={cn('shrink-0 ds-text-10 tabular-nums', isModified ? 'text-foreground' : 'text-muted-foreground/70')}
              aria-label={`Effective amount, wallet: ${walletDisplay}`}>
              {effectiveDisplay}
            </span>
          </TooltipTrigger>
          <TooltipContent side="top" className="ds-text-11">Wallet: {walletDisplay}</TooltipContent>
        </Tooltip>
      )}
      <div className="relative flex-1 min-w-0">
        {hasWallet && (
          <button type="button" onClick={toggleDeltaSign}
            className={cn('absolute left-1 top-1/2 -translate-y-1/2 z-10 rounded-sm px-0.5 ds-text-11 font-bold transition-colors', isPositiveDelta ? 'text-emerald-600 hover:bg-emerald-500/10' : 'text-red-500 hover:bg-red-500/10')}
            aria-label={isPositiveDelta ? 'Adding to position' : 'Reducing position'}>
            {isPositiveDelta ? '+' : '−'}
          </button>
        )}
        <input ref={numberInput.inputRef} value={numberInput.displayValue} onChange={numberInput.handleChange} onFocus={numberInput.handleFocus} onBlur={numberInput.handleBlur}
          inputMode="decimal" placeholder={hasWallet ? '' : (sideData.inputMode === 'usd' ? '10,000' : '100')}
          className={cn('h-[var(--ds-chip-h)] w-full min-w-[4rem] rounded-md ds-text-12 tabular-nums placeholder:italic', hasWallet ? 'pl-5 pr-7' : hasValue ? 'pl-2 pr-7' : 'pl-2 pr-2', cnDsInputSurface(hasValue, inputVariant))}
          aria-label={`${sideLabel} ${hasWallet ? 'delta' : 'amount'} for ${tokenSymbol}`} />
        {hasValue && (
          <button type="button" onClick={() => handleDeltaCommit('')}
...
function SideInputB(props: SideInputPrototypeProps) {
  const logic = useSideInputLogic(props);
  const { hasWallet, hasValue, isPositiveDelta, numberInput, handleDeltaCommit,
    toggleDeltaSign, handleToggleInputMode, effectiveDisplay, walletDisplay,
    isModified, priceUnavailable, isBorrow, labelColor, inputVariant,
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-1.5">
      ...
      {hasWallet && (
        <Tooltip>
          <TooltipTrigger asChild>
            <span className={cn('shrink-0 ds-text-10 tabular-nums inline-flex items-center gap-0.5', isModified ? 'text-foreground' : 'text-muted-foreground/70')}
              aria-label={`Wallet value, effective: ${effectiveDisplay}`}>
              <Lock className="size-2.5 opacity-60" aria-hidden />
              {walletDisplay}
            </span>
          </TooltipTrigger>
          <TooltipContent side="top" className="ds-text-11">
            {isModified ? `→ ${effectiveDisplay}` : 'Wallet position (unmodified)'}
          </TooltipContent>
        </Tooltip>
      )}
      <div className="relative flex-1 min-w-0">
...
function SideInputC(props: SideInputPrototypeProps) {
  const logic = useSideInputLogic(props);
  const { hasWallet, hasValue, isPositiveDelta, numberInput, handleDeltaCommit,
    toggleDeltaSign, handleToggleInputMode, effectiveDisplay, walletDisplay,
    isModified, priceUnavailable, isBorrow, labelColor, inputVariant,
  ...
  const walletPlaceholder = hasWallet ? walletDisplay : (sideData.inputMode === 'usd' ? '10,000' : '100');
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-1.5">
      ...
      <div className="relative flex-1 min-w-0">
        ...
        <input ref={numberInput.inputRef} value={numberInput.displayValue} onChange={numberInput.handleChange} onFocus={numberInput.handleFocus} onBlur={numberInput.handleBlur}
          inputMode="decimal" placeholder={walletPlaceholder}
          className={cn('h-[var(--ds-chip-h)] w-full min-w-[4rem] rounded-md ds-text-12 tabular-nums placeholder:italic', hasWallet ? 'pl-5 pr-7' : hasValue ? 'pl-2 pr-7' : 'pl-2 pr-2', cnDsInputSurface(hasValue, inputVariant))}
          aria-label={`${sideLabel} ${hasWallet ? 'delta' : 'amount'} for ${tokenSymbol}`} />
        {hasValue && (
          <button type="button" onClick={() => handleDeltaCommit('')}
      ...
      {hasWallet && hasValue && isModified && (
        <Tooltip>
          <TooltipTrigger asChild>
            <span className={cn('shrink-0 ds-text-10 tabular-nums text-muted-foreground/70 inline-flex items-center gap-0.5')}
              aria-label={`Effective amount after delta`}>
              → {effectiveDisplay}
            </span>
          </TooltipTrigger>
          <TooltipContent side="top" className="ds-text-11">
            Wallet: {walletDisplay} → {effectiveDisplay}
          </TooltipContent>
        </Tooltip>
      )}
    </div>
...
const VARIANT_MAP = { A: SideInputA, B: SideInputB, C: SideInputC } as const;
...
interface PortfolioTokenRowPrototypeProps {
...
const PortfolioTokenRowPrototype = memo(function PortfolioTokenRowPrototype({
  entry, actions, reserveId, tokenPriceInUsd, disabledNotice, capWarnings, variant,
}: PortfolioTokenRowPrototypeProps) {
  ...
  const SideInputComp = VARIANT_MAP[variant];
  ...
  const minusBtn = (
    <button type="button" onClick={isRestricted ? undefined : handleMinusClick}
      className={cn('shrink-0 rounded-md p-1 text-muted-foreground/60 transition-colors', !isRestricted && PORTFOLIO_THEME.trashHoverBg, !isRestricted && PORTFOLIO_THEME.trashHoverText)}
  ...
  const sideInputCommon = { tokenSymbol, tokenPriceInUsd, isMobile, reserveId, actions, variant };

  if (isMobile) {
    return (
      <div data-reserve-id={reserveId}
        className={cn('grid grid-cols-subgrid col-span-2 items-center gap-x-1 rounded-lg border transition-colors',
          isHidden ? 'border-border/20 bg-muted/5 opacity-40 hover:opacity-60' : 'border-border/50 bg-card/80 hover:border-border',
        ...
        <div className="flex flex-col items-stretch gap-1">
          <SideInputComp sideData={entry.supply} side="supply" sideLabel="Supply" disabled={!!disabledNotice?.supply} disabledNotice={disabledNotice?.supply} capLimitUsd={supplyCapLimitUsd} {...sideInputCommon} />
          {supplyWarnings && supplyWarnings.length > 0 && <CapWarningRow warnings={supplyWarnings} side="supply" isMobile={isMobile} />}
          <SideInputComp sideData={entry.borrow} side="borrow" sideLabel="Borrow" disabled={!!disabledNotice?.borrow} disabledNotice={disabledNotice?.borrow} capLimitUsd={borrowCapLimitUsd} {...sideInputCommon} />
  ...
  return (
    <div data-reserve-id={reserveId}
      className={cn('grid grid-cols-subgrid col-span-2 items-center gap-x-1 rounded-lg border transition-colors',
        isHidden ? 'border-border/20 bg-muted/5 opacity-40 hover:opacity-60'
          : entry.isOrphan ? 'border-border/20 bg-muted/5 opacity-60'
...
export default PortfolioTokenRowPrototype;
