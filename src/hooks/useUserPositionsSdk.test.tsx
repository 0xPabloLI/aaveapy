...
function makeReserve(
  overrides: Partial<ReserveWithSpread> & { marketName: string; chainId: number; tokenAddress: string },
): ReserveWithSpread {
  return {
    ...
    tokenName: 'Token',
    tokenSymbol: 'TKN',
    // marketName / chainId / tokenAddress come solely from ...overrides —
    // the earlier explicit copies were dead (always overwritten by the spread).
    ...overrides,
    ...
    const result = enrichV4SupplyPositions([
      {
        ...
        reserve: {
          ...
          spoke: {
            address: SPOKE,
            chain: { chainId: 42161 },
            connectedHubs: [{ hub: { name: 'Core', address: HUB } }],
          },
...
describe('buildV4ChainIds', () => {
  ...
  it('skips V3 reserves', () => {
    const reserves = [makeReserve({ marketName: 'AaveV3Ethereum', chainId: 1, tokenAddress: USDC })];
...
describe('useUserPositionsSdk - urql refetch on refetchEvent (S4, AAV-698)', () => {
  ...
  it('re-subscribes with a fresh address when the wallet address changes', () => {
    ...
    const { rerender, unmount } = renderHook(() => useUserPositionsSdk([], {}, {}), { wrapper });
