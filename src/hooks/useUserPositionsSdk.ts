...
import { UserSuppliesQuery as V4UserSuppliesQuery, UserBorrowsQuery as V4UserBorrowsQuery } from '@aave/graphql';
...
import {
  fetchFallbackPositions,
  type FallbackPositionsConfig,
  type FallbackLabels,
} from '@/lib/userData/fallbackPositions';
import {
...
export function enrichV3SupplyPositions(
  positions: {
    market: { address: `0x${string}`; chain: { chainId: number; [k: string]: unknown }; [k: string]: unknown };
    currency: { address: `0x${string}`; symbol: string; decimals: number; chainId: number; [k: string]: unknown };
    balance: { amount: { value: string; raw: string; decimals: number; [k: string]: unknown }; [k: string]: unknown };
  ...
  return positions.map((p) => ({
    ...
    balance: {
      amount: {
        value: p.balance.amount.value,
        onChainValue: BigInt(p.balance.amount.raw || '0'),
        decimals: p.balance.amount.decimals,
      },
    },
...
export function enrichV3BorrowPositions(
  positions: {
    market: { address: `0x${string}`; chain: { chainId: number; [k: string]: unknown }; [k: string]: unknown };
    currency: { address: `0x${string}`; symbol: string; decimals: number; chainId: number; [k: string]: unknown };
    debt: { amount: { value: string; raw: string; decimals: number; [k: string]: unknown }; [k: string]: unknown };
    [k: string]: unknown;
  }[],
): SdkBorrowPosition[] {
  return positions.map((p) => ({
    ...
    debt: {
      amount: {
        value: p.debt.amount.value,
        onChainValue: BigInt(p.debt.amount.raw || '0'),
        decimals: p.debt.amount.decimals,
      },
    },
  }));
}
...
export function enrichV4SupplyPositions(
  positions: {
    ...
    reserve: {
      ...
      spoke: {
        address: `0x${string}`;
        chain: { chainId: number; [k: string]: unknown };
        connectedHubs?: { hub: { name: string; address?: string } }[];
        [k: string]: unknown;
      };
      summary: {
        supplied: {
          token: {
            address: `0x${string}`;
            info: { symbol: string; decimals: number; [k: string]: unknown };
            [k: string]: unknown;
          };
          [k: string]: unknown;
        };
    ...
    balance: {
      amount: { value: string; onChainValue: bigint; decimals: number; [k: string]: unknown };
      [k: string]: unknown;
    };
    isCollateral: boolean;
    [k: string]: unknown;
  ...
  return positions.map((p) => ({
    reserve: {
      ...
      underlyingAsset: {
        address: p.reserve.summary.supplied.token.address,
        chain: { id: String(p.reserve.spoke.chain.chainId) },
      },
    ...
    balance: {
      amount: {
        value: p.balance.amount.value,
        onChainValue: p.balance.amount.onChainValue,
        decimals: p.balance.amount.decimals,
      },
    },
    isCollateral: p.isCollateral,
...
export function enrichV4BorrowPositions(
  ...
  return positions.map((p) => ({
    reserve: {
      ...
      underlyingAsset: {
        address: p.reserve.summary.borrowed.token.address,
        chain: { id: String(p.reserve.spoke.chain.chainId) },
      },
      spokeAddress: p.reserve.spoke.address,
      hubName: p.reserve.spoke.connectedHubs?.[0]?.hub.name,
    ...
    debt: {
      amount: {
        value: p.principal.amount.value,
        onChainValue: p.principal.amount.onChainValue,
        decimals: p.principal.amount.decimals,
      },
    },
  }));
}
...
export function buildV4ChainIds(reserves: ReserveWithSpread[]): number[] {
...
export function buildV3SdkArgs(
  ...
  if (!enabled || !account || v3MarketInputs.length === 0) {
    return {
      markets: [{ address: evmAddress('0x0000000000000000000000000000000000000000'), chainId: chainId(1) }],
...
export function buildV4SdkArgs(enabled: boolean, account: `0x${string}` | undefined, v4ChainIds: number[]) {
  if (!enabled || !account || v4ChainIds.length === 0) {
    return {
      query: { userChains: { user: evmAddress('0x0000000000000000000000000000000000000000'), chainIds: [chainId(1)] } },
    };
  }
...
export function useUserPositionsSdk(
  ...
  const gapChainIds = useMemo(() => computeGapChainIds(reserves, sdkCoverage), [reserves, sdkCoverage]);
  ...
  useEffect(() => {
    ...
    return subscribeRefetch(() => {
      ...
      const matchesV4User = (variables: {
        request: { query: { userChains?: { user: unknown }; userSpoke?: { user: unknown } } };
      }) => {
  ...
  if (!v3SdkFailed && v3Supplies.data && v3Borrows.data) {
    sdkPositions.push(
      ...convertSdkSuppliesToWalletPositions(
        enrichV3SupplyPositions(v3Supplies.data as unknown as Parameters<typeof enrichV3SupplyPositions>[0]),
        sdkReserveMap,
        sdkLookupMap,
        'sdk',
      ),
    ...
    sdkPositions.push(
      ...convertSdkBorrowsToWalletPositions(
        enrichV3BorrowPositions(v3Borrows.data as unknown as Parameters<typeof enrichV3BorrowPositions>[0]),
        sdkReserveMap,
        sdkLookupMap,
        'sdk',
      ),
  ...
  if (!v4SdkFailed && v4Supplies.data && v4Borrows.data) {
    sdkPositions.push(
      ...convertSdkSuppliesToWalletPositions(
        enrichV4SupplyPositions(v4Supplies.data as unknown as Parameters<typeof enrichV4SupplyPositions>[0]),
        sdkReserveMap,
        sdkLookupMap,
        'sdk',
      ),
    ...
    sdkPositions.push(
      ...convertSdkBorrowsToWalletPositions(
        enrichV4BorrowPositions(v4Borrows.data as unknown as Parameters<typeof enrichV4BorrowPositions>[0]),
        sdkReserveMap,
        sdkLookupMap,
        'sdk',
      ),
