...
- In simulation incentive breakdown rows, avoid showing opaque identifiers (e.g. `campaignId` suffixes) or campaign-type labels when they don’t help users; when multiple rows would otherwise be indistinguishable, use a simple `#1/#2/...` disambiguator only in the no-scenario-input state, and prefer placing outbound incentive links on the specific expanded row rather than the aggregate row.
