...
- **`table-layout: fixed` + `auto` 列 = 剩余空间全给 auto 列**：`table-layout: auto` 下无 width 的列会吸收剩余空间（与 `fixed` 一样）。正确方案：`table-layout: fixed`，Token 列固定 `72px`，Input 列设 `auto`——`fixed` 下 `auto` 列吸收所有 fixed 列分配后的剩余空间。**约定：多余空间全留给 Input 列（用户输入区域），Token 列固定窄宽度。** **教训：`table-layout` 的 `auto` 和 `fixed` 下，无 width / `auto` 列都会吸收剩余空间——区别在于 `fixed` 只看第一行宽度，`auto` 看所有内容。控制列宽分配的正确方式是显式设 width + `fixed`，让 `auto` 列精确吸收剩余。**
...
- **Input placeholder 显示 wallet position**：当有 wallet position 且输入框为空时，placeholder 显示 wallet 的完整值（如 `$1,000`），而非固定占位符（`10K`）。箭头在输入为空时自动 muted（因为 effective = wallet → delta = 0）。**教训：placeholder 是最好的"暗示"位置——用户看到 `$1,000` 就知道这是当前仓位，输入新值就是目标仓位，无需额外说明。**
- **`?unified=0` opt-out 移除——unified 是唯一布局**：legacy `PortfolioTokenRow` + `PortfolioTokenRowPrototype` + `PortfolioSummaryCard` + `PortfolioResultsTable` 全部从 `PortfolioPanel` 移除。`unifiedMode` flag 删除，`?unified=0` 不再可用。**教训：feature flag 从 opt-out 转"唯一模式"时，必须删除所有 flag 引用 + 清理 dead imports + 更新/删除测试 flag 的测试用例——不能留 flag 在代码里"以防万一"。**
