...
- `npm run ci:remote` — full local gate (used by hooks; **AI agent MUST run this manually before every `git push`** because husky hooks may not fire in non-interactive bash environments)
