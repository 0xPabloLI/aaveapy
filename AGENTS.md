...
- **UI Geometry & Debugging**: When addressing visual seams, residual lines, or discontinuity in UI geometry, **fix the root architectural cause** (e.g., ensure a single-source-of-truth contour) instead of iteratively applying localized patches, masks, or visually tuning Bezier curves. The user strongly dislikes "patch-on-top-of-patch" workarounds.
- For SVG-based 1px stroke rendering, align coordinates to half-pixels (e.g. 0.5) to avoid anti-aliasing blur. When grafting custom SVG contours onto native CSS borders, avoid layered masking; instead, use `clip-path` to cut out the native border and draw a single continuous SVG path using exact geometric commands (e.g. `A` for arcs) rather than manual Bezier curves.
