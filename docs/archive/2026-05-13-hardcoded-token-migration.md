...
>
> **对齐契约**：`--ds-control-h` 与 `--ds-seg-track-h` 的值必须保持同步（当前均为 `2rem`）。设计规范 [§5.1](file:///Users/pabloli/Documents/code/aaveapy/docs/design/DESIGN-SYSTEM-REFERENCE.md#L341-L366) 明确要求分段控制器高度与同行输入框对齐 — 这两个 token 是同一物理约束的两个语义表述。在 `:root` 中添加注释标注此约束：

```css
/* --ds-control-h 与 --ds-seg-track-h 值必须保持同步，确保分段控制器与同行输入框对齐 */
--ds-control-h: 2rem;
--ds-seg-track-h: 2rem;
```
...
- **特殊场景指南**：当元素已有显式 `leading-none` 且不希望被 `ds-text-N` 的 line-height 覆盖时，使用 `ds-text-N !leading-none` 组合（`!leading-none` 的 `!important` 覆盖 line-height）
- **更特殊场景指南**：当需要在 shadcn 组件（如 `<Input>`）上覆盖 font-size 且避免 line-height `!important` 冲突时，使用 `![font-size:var(--ds-text-N)]` 任意属性语法
...
- `ds-text-N !leading-none` 适用于大多数场景：专用工具类可读性好，`!leading-none` 显式声明 "我覆盖了 ds-text-N 的 line-height"
- `![font-size:var(--ds-text-11)]` 适用于 shadcn 组件覆盖场景：需要 `!important` 在 font-size 上但必须避免 line-height `!important`

> 项目中已有先例：[InkAprCalculator L474](file:///Users/pabloli/Documents/code/aaveapy/src/components/dashboard/InkAprCalculator.tsx#L474) 使用 `[font-size:11px]`，证明此语法可在当前 Tailwind 构建中正常工作。

### 第 1 层：className 断言回归守卫（Vitest 组件测试）

**目的**：防止后续维护中有人将 `ds-text-* !leading-none` 改回 `text-[Npx]` 或将 Input 的 `![font-size:var(--ds-text-11)]` 改回 `!ds-text-11`。
...
describe('InkAprCalculator font-size token regression guard', () => {
  ...
  it('FDV Input uses ![font-size:var()] not !ds-text-11', () => {
    const html = document.body.innerHTML;
    // Input 必须用任意属性语法（唯一安全方案用于 shadcn 组件覆盖）
