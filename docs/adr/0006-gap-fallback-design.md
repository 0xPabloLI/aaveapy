...
SDK 成功返回但覆盖不全（address-book 含链而 SDK 不含）时，触发 gap fallback 补齐差集链仓位。与 onchain fallback（SDK 失败时全量替代，ADR-0003）互补，组成三路合并架构。

## Context

`buildV3MarketInputs` 中 `if (!pool) continue` 对 `V3_POOL_ADDRESSES` 未覆盖的链静默跳过，SDK 不报错也不 fallback，用户在该链上的仓位无感知丢失（AAV-451）。onchain fallback（ADR-0003）仅在 SDK 整体失败时触发，无法覆盖"SDK 成功但覆盖不全"的中间态。
...
- SDK 失败（`isInfrastructureFailure`）→ 走 onchain fallback（全量替代）
- 两者互斥，不会同时触发
...
三路数据（SDK / onchain fallback / gap fallback）独立管理，触发条件不同不耦合：
- SDK: `useV3UserSupplies` / `useV4UserSupplies` 等
- Onchain fallback: `useQuery` triggered by `isInfrastructureFailure`（ADR-0003）
...
| 维度 | Onchain Fallback (ADR-0003) | Gap Fallback |
|---|---|---|
...
reserves (from /markets API)
    ...
    │       └─ SDK failure → trigger onchain fallback useQuery (ADR-0003)
...
- Gap fallback 与 onchain fallback 互斥，不会重复查询
- 后续可收拢（I-1）：提取共享 `fetchFallbackPositions(config, { chainFilter? })`，消除 gap 与 onchain fallback 的逻辑重复（当前 gap 版有 chainFilter 语义差异，暂独立）
