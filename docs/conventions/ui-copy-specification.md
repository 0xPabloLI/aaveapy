...
- The `offsetNotes` field is rendered on every campaign row under the same source (pragmatic: offset is source-level but `hideAggregateWhenCampaigns` hides the source-level row).
