...
| `compare-merit-forecast-paths.mjs` | `MERIT_COMPARE_API_BASE` → constant | **Staging** |
...
To compare against **production** from a laptop or CI that can reach it, set the relevant env var explicitly (e.g. `MERIT_COMPARE_API_BASE`, `VITE_API_BASE_URL`, or `LIVE_TEST_API_BASE_CI`).
