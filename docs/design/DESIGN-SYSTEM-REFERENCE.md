...
- 主色 / 强调色：用于 CTA、选中态。
- 数据/状态色：成功（绿）、警告（琥珀）、错误（红）、冻结（天蓝/sky-500）— 仅用于对应语义，不用于普通数据展示。sky-500 仅用于 Frozen 资产，不用于其他"信息/更新"语义。
...
圆点指示器（freshness dot、连接状态点等）统一使用 `-400` 色阶；图标+文字用 `-500`/`-600`；背景用 `-50`~`-500/10`。

| 形态 | 色阶 | 理由 |
|------|------|------|
| 圆点（纯色小圆，≤ 10px） | `-400` | 小面积需更高饱和度才可见 |
