...
- **浮层**：移动端详情类内容用**底部抽屉（bottom sheet）**：全宽、`rounded-t-2xl`、固定标题+关闭、内容区 `max-h-[80vh] overflow-y-auto`，背景 `fixed inset-0 z-30 bg-background/20` 点击关闭。标题栏用 `px-[var(--ds-space-3)] py-[var(--ds-space-2)]` 紧凑间距（与桌面 Tooltip 的 `ds-tooltip-pad` 12px 分层，移动端减少标题栏高度让更多空间留给内容区）。不在移动端用小浮层 popover 锚定在触发点上。
