...
- **Incentive tooltip vs shared simulation**: `IncentiveTooltip` shows **static** incentive context (campaign dates, messages, Merkl whitelist opt-in). **Deposit- and TVL-dependent** forecasts (Merkl hypothetical TVL, Merit Self position-cap lines, FIX rewardable horizon, Brevis position cap / eligible-amount lines, cap-binding warnings, etc.) belong in the **shared rate simulation** UI (`useRateSimulation` per-campaign rows on `SimulationSubRow` via `capNote` / `capWarning`), not inside the tooltip. Merit **Base** and Merkl **DUTCH_AUCTION** use **no** row `capNote` (scenario APR only); keep that policy in sync if it changes. **New** user-visible cap lines should be produced via `src/lib/incentiveCaps.ts` (then mapped to `capNote` / `capWarning`) where applicable—see `docs/rate-calculation.md` (Incentive Cap Reference, naming layers).
...
- **TARGET_TOTAL_APR tooltip formula APR/APY behavior**: The `Target total: X% = Native Y% + Merkl Z%` formula in `IncentiveTooltip` follows the global APR/APY toggle for **all three terms** (Target total, Native, Merkl), keeping them in the same unit so the equation holds. Rationale: mixing units within the formula (e.g. Target in APR + Native in APY) or diverging from the tooltip's outer campaign values would be more confusing than the minor APY additivity approximation. In **APR mode** the formula is mathematically exact; in **APY mode** the sum is a close approximation (APY is not strictly additive under compounding, but at typical incentive magnitudes the error is negligible). This differs from the official Aave interface (`aave/interface` `useMerklIncentives.ts`), which has no APR/APY toggle and always displays in APY—so the issue never arises there.
...
| Topic | Behavior |
|-------|------------|
| **Default** | **No** whitelist-only campaigns are included in totals. App state is `whitelistMerklCampaignIds: Set<string>` on `Index`, initially **empty**. When a reserve's **only** incentive is whitelist-only and unchecked, the APR badge displays **0** (not hidden) — the tooltip still shows the incentive source with the raw APR greyed out and a WL opt-in checkbox. |
...
| **Implementation** | `isMerklWhitelistBreakdownIncluded()` and `MERKL_WHITELIST_NO_CAMPAIGN_ID_SENTINEL` in `merklWhitelist.ts`; `useRateSimulation` / `useSharedRateSimulations`; `collectMerklCampaignOptions` / `collectWhitelistOnlyMerklCampaignEntries` in `merklCampaigns.ts`. **Auto campaign access detection**: `useCampaignAccess(userAddress)` in `useCampaignAccess.ts` computes `campaignAccessStatuses: Record<campaignId, CampaignAccessStatus>` via `computeCampaignAccessStatuses()`, passed to `ReservesTable`, `IncentiveTooltip`, `TopOpportunities`, and `rateSimulationCalculator` to auto-exclude `whitelist-blocked`/`blacklisted` campaigns based on the connected wallet address. |
...
2. **Do** keep the **reference structure** in `ReservesTable.tsx`:
   - **Desktop reserves card shell**: outer wrapper uses **`rounded-2xl bg-border/60 p-px`** plus an inner **`rounded-[calc(1rem-1px)] bg-card`** that holds scenario + table. This **1px gutter** draws a continuous outline without `overflow: hidden` (which would break viewport `sticky`). It avoids the common bug where a full-bleed sticky child’s opaque **`bg-card`** paints **over** the parent’s native **`border`** so **top rounded corners look clipped**—see **DESIGN-SYSTEM-REFERENCE** § 轮廓与圆角拼接 (structural fix, not mask stacks).
    - **`data-reserves-sticky-scenario`**: shared scenario strip, **`bg-card`** (same opaque surface as sticky **`th`** headers so the control bar and column headers read as one continuous card top—avoid **`bg-muted/…` + backdrop blur on this wrapper**, which fights the thead). **Sticky behavior varies by mode** (ADR-0013):
      - **Single mode (desktop + mobile)**: `sticky top-0 z-20` — scenario bar pins at viewport top.
      - **Portfolio mode (desktop + mobile)**: **no sticky** — scenario bar scrolls naturally with the page. PortfolioPanel (search + suggested chips + many position rows + summary) can exceed viewport height; sticky would make overflow content unreachable. `--reserves-sticky-scenario-height` is set to `0px` in Portfolio mode so `<thead>` (`top: 0`) and expanded rows (`top: theadHeight`) still pin correctly.
      **`ScenarioControls`** on desktop may use a **borderless** tinted well (`bg-card/60`, `backdrop-blur-sm`) inside the padded strip; do **not** add a second full **`border`** around the entire control row (the reserves card gutter already frames the block). **Supply / Borrow**: labels keep emerald / brand cyan; **empty** inputs use **neutral** border (`border-border/…`) + transparent fill (no semantic border hue until filled); **filled** tint matches border hue via **`cnDsInputSurface`**—see **DESIGN.md** §4.1 / §4.8.
    - **Mobile scenario strip: Portfolio & expand icon per-section centering (normative)**: On mobile, the right column beside `ScenarioControls` contains the Portfolio toggle (`PortfolioModeToggle`) above and the expand icon (`SlidersHorizontal`) below. These **must** each be vertically centered within their respective halves of the strip, matching the two visual rows of `ScenarioControls`: (1) **upper half** = input row (USD/Token segmented control + Supply/Borrow inputs), (2) **lower half** = Net lending & borrowing checkbox area (visible when `mobileNetOpen`). Implementation: outer row uses `flex items-stretch` (not `items-start`) so the right column stretches to the full height of `ScenarioControls`; the right column is split into two `flex-1` blocks, each with `flex items-center justify-center`. When the Net panel is collapsed (`mobileNetOpen === false`), the lower block has zero height and the expand icon sits directly below the Portfolio toggle. Do **not** revert to a single `flex-col gap-1.5` container that stacks both controls without per-section centering.
    - **Mobile scenario strip: expand icon position stability (normative)**: When the Net lending & borrowing panel opens/closes inside `ScenarioControls`, the Portfolio toggle and expand icon **must not move**. Implementation: the Net panel uses `position: absolute` (`absolute left-0 right-0 top-full z-10`) so it renders below the input row without affecting `ScenarioControls`'s in-flow height. The parent `<div>` of `ScenarioControls` mobile render uses `relative` to anchor this absolute panel. Do **not** revert the Net panel to in-flow layout (e.g. `mt-1.5 px-0.5 pb-0.5`), as that would cause `ScenarioControls` height to change when the panel opens, shifting the right column's Portfolio toggle and expand icon positions. This rule and the per-section centering rule above are **simultaneously required**: absolute positioning keeps positions stable; `items-stretch` + two `flex-1` centering blocks keep each control centered in its half.
...
- **Mobile hub badge is filter-only (no external link arrow)**: do not overload the tiny hub badge on mobile with both filter and navigation. On mobile, the hub pill is a pure filter `<button>` (calls `onSelectHub`). The external link to the Aave V4 hub page is omitted for space reasons; hub filtering is triggered from the filter bar (hub chips in hub view mode). Desktop shows the `<ExternalLink>` arrow next to the hub pill (hover-revealed), linking to `pro.aave.com/explore/hub/{hubId}`.
...
## F. Portfolio Mode Simulation (Portfolio Toggle)
