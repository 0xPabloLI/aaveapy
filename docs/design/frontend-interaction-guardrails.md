...
- **Mobile overlays (弹框) must use bottom-sheet style, not floating popover**:
  ...
  - **Use the shared `BottomSheet` component** (`src/components/dashboard/BottomSheet.tsx`) to avoid duplicating the shell code. Do not pass `animate` or `showShadow` to mobile bottom sheets — the `animate` prop can cause framer-motion visual artifacts on low-end devices, and `showShadow` (`ds-tooltip-shadow-up`) adds a large upward shadow that may be perceived as an extra popup.

### B. 移动端 tooltip 禁用模式 (`disableTooltip`)

**When a component renders both a Radix Tooltip (desktop) and a BottomSheet (mobile)**,
the mobile render path must avoid having the Radix Tooltip compete with the BottomSheet.
The standard pattern is a `disableTooltip` boolean prop:
...
// Mobile caller — opens BottomSheet on parent button click
<CapProgressRing disableTooltip />

// Desktop caller — tooltip on hover
<CapProgressRing />
```

**Enforcing components** (must support `disableTooltip`):
| 组件 | `disableTooltip` guard 行号 |
|---|---|
| `CapProgressRing.tsx` | L125 |
| `BorrowCapProgressRing.tsx` | L137 |
| `DeficitLiquidityRing.tsx` | guard 已存在 |
| `UtilizationIndicator.tsx` | L96 |

**`MobileReserveCard` 合规检查清单**（每次改卡片时必须验证）：
- [ ] `grep -n '<Tooltip' src/components/dashboard/MobileReserveCard.tsx` 返回 **0 条结果**
- [ ] 所有 `CapProgressRing` / `BorrowCapProgressRing` / `DeficitLiquidityRing` / `UtilizationIndicator` 均传入 `disableTooltip`
- [ ] Frozen/Paused 徽章不使用 `<Tooltip>` 包裹（直接用 `button` + `onClick → BottomSheet`）
- [ ] Supply/Borrow unavailable 不用 `<TooltipContent>` 包裹（用内联 `<span>` 替代）
- [ ] **硬刷新浏览器**（Cmd+Shift+R）后验证，确认无 Radix Tooltip popover 与 BottomSheet 同时出现

> **背景**：2026-05 移动端卡片的 Dirty Tooltip Popup Bug — 代码层面移除所有 Radix
> Tooltip 后，离线测试通过（Playwright 零 popup），但用户端仍看到脏弹窗。
> 根因：**浏览器缓存了旧 JS bundle**，硬刷新后问题消失。
