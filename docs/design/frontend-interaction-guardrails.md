...
5. **Scroll implementation** (`src/lib/scrollExpandedSimulationIntoView.ts`): desktop `pin-main-row-top` uses `window.scrollBy` so `tr[data-reserve-id]` top aligns to `getPinnedRowTopY()` = `max(bottom of [data-reserves-sticky-scenario], bottom of [data-reserves-sticky-thead]) + GAP_BELOW_STICKY_STACK_PX`, else `VIEW_MARGIN_PX`. Mobile expanded block: `[data-reserve-expanded-anchor]`. Do not remove these `data-*` hooks from the sticky wrappers without updating this function and this doc. **Expanded main-row sticky** (`DesktopReserveRow` sticky `td`, `top: var(--reserves-expanded-main-row-top)`) must stay **geometrically consistent** with the same stack: that variable is **scenario height + thead height** (px) on the card—do not change one without the other. **Simulation inner scroll**: the `SimulationSubRow` container uses `overflow-y: auto` with `max-height: calc(100dvh - var(--reserves-expanded-main-row-top) - mainRowHeight - 1rem)` so all simulation content is scrollable within its own pane and never hidden behind the sticky main row. Filter-triggered pin scroll uses `instant` behavior (not `smooth`) to avoid race conditions with DOM reflow.
