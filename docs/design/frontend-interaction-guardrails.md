...
- **TARGET_TOTAL_APR tooltip formula APR/APY behavior**: The `Target total: X% = Native Y% + Merkl Z%` formula in `IncentiveTooltip` must keep all three terms in the same unit (all APR or all APY) so the equation holds. In **APR mode** the formula is mathematically exact (`targetAPR = nativeAPR + merklAPR`). In **APY mode** the sum is a **close approximation** (APY is not additive under compounding, but at typical incentive magnitudes the error is negligible). This differs from the official Aave interface (`aave/interface` `useMerklIncentives.ts`), which has no APR/APY toggle and always displays everything in APY. Our design choice: in the tooltip formula, Native follows the toggle (via `apyToApr`) to preserve the additive decomposition, even though table cells elsewhere always show Native in APY.
